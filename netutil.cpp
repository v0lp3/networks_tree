...
int parse_int(const string number)
{
...
string int_to_octet(int integer)
{
    ...
    while (integer > 0)
    {
        ...
        octet = to_string(integer % 2) + octet;
        integer /= 2;
    }
...
string ip_to_bin(const string address)
{
    ...
    for (int i = 0; i < address.length(); i++)
    {
        if (address[i] == '.')
        {
            bits += int_to_octet(parse_int(tmp_number));
            tmp_number = "";
        }
        else
        {
...
string get_bin_prefix(const string address, const int prefix_len)
{
...
string complete_octet(const string base_address, const string path, bool mode)
{
